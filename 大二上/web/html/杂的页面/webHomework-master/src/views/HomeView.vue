  ...
  <div>
    ...
    <div class="main-container">
      ...
      <div class="nav-right" data-url="/visit-nav/">
        ...
        <div class="nav-list">
          <h2 id="近期更新">近期新增</h2>
          <div v-for="(item,index) in articleList.slice(0,20)" :key="index">
            <myBox :item="item"></myBox>
          </div>
        </div>
        <div class="nav-list">
          <h2 id="资源搜索">资源搜索</h2>
          <div v-for="(item,index) in articleList.slice(21,40)" :key="index">
            <myBox :item="item"></myBox>
          </div>
        </div>
      </div>
...
import {selectAllArticles, setFavorite} from "@/api/serveArticle";
import NavLeft from "@/components/home/navLeft";

export default {
  name: "myMain",
  components: {NavLeft, myBox, myFoots, tools, boottomTips, headBanner, headerNav},
  data() {
    return {
      articleList: []
    }
