...
public class NetInfoUtil {
	...
	public static List<String[]> getExcellentMenu() {// 获取推荐的精品菜品
		try {
			cacheConnect();   //连接服务器
			//向服务器发送参数值：Constant.GET_RECOMMEND_MENU，请求“精品菜品”
			//服务器端ServerAgentThread通过识别参数值Constant.GET_RECOMMEND_MENU，确定调用服务器端的DBUtil.getExcellentMenu()
			//DBUtil.getExcellentMenu()执行SQL语句“select menu_id from menu_recommend order by r_time”来返回精品菜品
			cachedos.writeUTF(Constant.GET_RECOMMEND_MENU);
			message = cachedin.readUTF();  //从服务器返回从数据库中获取的“精品菜品”
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			cacheDisConnect();
		}
		return StrListChange.StrToList(message);   //将“精品菜品”由字符串转化为list并返回。
	}
