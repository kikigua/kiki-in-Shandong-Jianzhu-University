		...
		singleThreadExecutor.execute(new Runnable() {			//网络加载
			...
			public void run() {
				try {
					...
					final Bitmap bitmap = BitmapFactory.decodeByteArray(bb, 0,bb.length);
					if (bitmap != null) {
						//将下载的图片保存到图片缓存中
						BitmapCache.addBitmapToMemoryCache(thumbnail + picName,bitmap);
						mHandler.post(new Runnable() {
							@Override
							public void run() {
								String target=(String) view.getTag();
