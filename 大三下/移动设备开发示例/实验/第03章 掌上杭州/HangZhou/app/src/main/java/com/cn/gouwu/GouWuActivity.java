...
public class GouWuActivity extends Activity
{
    ...
    int[] latitude;
	 ...
	 public void onCreate(Bundle savedInstanceState) 
	 {
	     ...
	     setContentView(R.layout.gouwu_main);
	      //用自定义的字体方法
	        FontManager.changeFonts(FontManager.getContentView(this),this);

	        GridView gridview = (GridView) findViewById(R.id.gv_gouwu);
		     
	        String information=PubMethod.loadFromFile("gouwu/name.txt");   //文本内容，获取图片名和菜名
	        infor=information.split("\\|");
	        final int count=infor.length/4;    //有多少道菜
		    ...
		    latitude=new int[count];
	         ...
	         {
	        	 namePP[i]=infor[i*4];
	        	 imgPath[i]="gouwu/"+infor[i*4+1]+".jpg";
	        	 jiePath[i]="gouwu/"+infor[i*4+1]+".txt";
	        	 longitude[i]=Integer.valueOf(infor[i*4+2]);
			     latitude[i]=Integer.valueOf(infor[i*4+3]);
			     
			     System.out.println("============"+ namePP[i]);
	         }
		...
		{  
		    ...
		    //选中项目，跳转到下一界面
		    Intent intent=new Intent(GouWuActivity.this,GyActivity.class);
